...
import { providerRoleIsEligibleForService } from "./scheduling-provider-eligibility.mjs";
...
export function recurringRevisionStatus(revision) {
  return String(revision?.status || "").trim() || "active";
}

export function activeRecurringSeriesRevisions(series) {
  return (Array.isArray(series?.revisions) ? series.revisions : [])
    .filter((revision) => recurringRevisionStatus(revision) === "active")
    .sort((left, right) => (
      String(left.effectiveStartDate || "").localeCompare(String(right.effectiveStartDate || ""))
      || String(left.id || "").localeCompare(String(right.id || ""))
    ));
}

export function governingRecurringSeriesRevision(series, effectiveDate) {
  if (!isValidDateOnly(effectiveDate)) return null;
  return activeRecurringSeriesRevisions(series).find((revision) => (
...
export function sanitizeRecurrenceRows(value) {
  ...
  if (value.length > 7) errors.push("At most seven recurrence rows are supported.");
...
export function expandActiveRecurringSeries({ series, startDate = series?.startDate, endDate = series?.endDate }) {
  const errors = [];
  if (!series || !String(series.id || "").trim()) errors.push("A recurring series with an id is required.");
  if (!isValidDateOnly(startDate)) errors.push("startDate must be a valid YYYY-MM-DD date.");
  if (!isValidDateOnly(endDate)) errors.push("endDate must be a valid YYYY-MM-DD date.");
  if (!errors.length && endDate < startDate) errors.push("endDate must be on or after startDate.");
  if (!isValidIanaTimeZone(series?.timeZone)) errors.push("A valid IANA timezone is required.");
  if (errors.length) return { occurrences: [], errors: [...new Set(errors)] };

  const occurrences = [];
  for (const revision of activeRecurringSeriesRevisions(series)) {
    const boundedStart = revision.effectiveStartDate > startDate ? revision.effectiveStartDate : startDate;
    const boundedEnd = revision.effectiveEndDate < endDate ? revision.effectiveEndDate : endDate;
  ...
  if (duplicateOriginalSlotIdentities(occurrences).length) {
    errors.push("Active recurring-series revisions generate duplicate original-slot identities.");
  }
  occurrences.sort((left, right) => (
    left.originalOccurrenceLocalDate.localeCompare(right.originalOccurrenceLocalDate)
    || left.recurrenceRowId.localeCompare(right.recurrenceRowId)
  ));
  return { occurrences: errors.length ? [] : occurrences, errors: [...new Set(errors)] };
}
...
function referenceErrors(series, { clients, users, requireActiveReferences }) {
  ...
  for (const [index, revision] of series.revisions.entries()) {
    if (recurringRevisionStatus(revision) === "superseded") continue;
    ...
    if (!provider) errors.push(`Revision ${index + 1} provider does not exist.`);
      ...
      if (!providerRoleIsEligibleForService(provider.role, revision.template.serviceCode)) {
        errors.push(`Revision ${index + 1} provider role is not permitted for the serviceCode.`);
      }
    }
  ...
  if (!Array.isArray(value?.revisions) || !value.revisions.length) {
    ...
    series.revisions = value.revisions.map((revision, index) => {
      ...
      const sanitized = {
        ...
        status: recurringRevisionStatus(revision),
        supersededAt: String(revision?.supersededAt || "").trim(),
        supersededByRevisionId: String(revision?.supersededByRevisionId || "").trim(),
        supersededByOperationId: String(revision?.supersededByOperationId || "").trim()
      };
      ...
      if (!REVISION_STATUSES.has(sanitized.status)) {
        errors.push(`Revision ${index + 1} status must be active or superseded.`);
      }
      if (sanitized.status === "active") {
        if (sanitized.supersededAt || sanitized.supersededByRevisionId || sanitized.supersededByOperationId) {
          errors.push(`Revision ${index + 1} is active and cannot include supersession metadata.`);
        }
      } else if (sanitized.status === "superseded") {
        if (!isValidTimestamp(sanitized.supersededAt)) {
          errors.push(`Revision ${index + 1} supersededAt must be a valid timestamp.`);
        }
        if (!sanitized.supersededByRevisionId) {
          errors.push(`Revision ${index + 1} requires supersededByRevisionId.`);
        }
        if (!sanitized.supersededByOperationId) {
          errors.push(`Revision ${index + 1} requires supersededByOperationId.`);
        }
      }
      return sanitized;
    });
    ...
    const revisionsById = new Map(series.revisions.map((revision) => [revision.id, revision]));
    for (const [index, revision] of series.revisions.entries()) {
      if (revision.status !== "superseded") continue;
      const replacement = revisionsById.get(revision.supersededByRevisionId);
      if (!replacement) {
        errors.push(`Revision ${index + 1} supersededByRevisionId must reference an existing revision.`);
      } else {
        if (replacement.id === revision.id) errors.push(`Revision ${index + 1} cannot supersede itself.`);
        if (replacement.effectiveStartDate !== revision.effectiveStartDate) {
          errors.push(`Revision ${index + 1} replacement must use the same effectiveStartDate.`);
        }
        if (isValidTimestamp(replacement.createdAt) && isValidTimestamp(revision.createdAt)
          && Date.parse(replacement.createdAt) < Date.parse(revision.createdAt)) {
          errors.push(`Revision ${index + 1} replacement must not predate the superseded revision.`);
        }
      }
    }
    for (const revision of series.revisions) {
      ...
      while (current?.status === "superseded" && current.supersededByRevisionId) {
        if (visited.has(current.id)) {
          errors.push("Series revision supersession references must not be circular.");
          break;
        }
        visited.add(current.id);
        current = revisionsById.get(current.supersededByRevisionId);
      }
    }
    const sorted = activeRecurringSeriesRevisions(series);
    if (!sorted.length) errors.push("Series requires at least one active revision.");
