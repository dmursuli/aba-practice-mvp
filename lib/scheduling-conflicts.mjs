...
function localDateTimeParts(timestamp, timeZone) {
  try {
...
function missingAvailabilityWarning(appointment, providerUserId, users) {
  ...
  try {
    occurrenceLocalDate = localDateTimeParts(
...
function appointmentFitsAvailability(appointment, profile) {
  const timeZone = String(profile.timezone || appointment.timeZone || "America/New_York");
  const start = localDateTimeParts(appointment.scheduledStartAt, timeZone);
  const end = localDateTimeParts(appointment.scheduledEndAt, timeZone);
...
export function validateProviderScheduling({
  ...
  proposed.forEach((appointment) => {
    providerIds(appointment).forEach((providerUserId) => {
      ...
      const appointmentDate = profile
        ? localDateTimeParts(appointment.scheduledStartAt, profile.timezone)?.date || ""
        : "";
      if (!profile || (appointmentDate && appointmentDate < profile.effectiveDate)) {
